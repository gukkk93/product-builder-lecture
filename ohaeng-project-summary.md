...
> **[3-11에서 대체됨]** 이 섹션이 설명하는 `explanation/goodFit/meetingScenario/situational/watchFor` 7개 고정 섹션 구조는 3-11에서 relationshipMode(친구/룸메이트/그룹케미) 선택형 구조로 전면 교체됨 — `idolMatchTemplates.js`/`dramaMatchTemplates.js`(이 섹션에서 만든 `goodFit`/`meetingScenario`/`pillarSituational` 포함)는 이제 `IdolMatch.jsx`/`DramaMatch.jsx` 어디에서도 안 쓰임(다른 파일에서도 참조 없음, 순수 주석 언급만 남음). 파일 자체는 콘텐츠 손실 방지를 위해 삭제하지 않고 레포에 남겨둠 — 필요 없다고 판단되면 삭제해도 되는 상태. 아래 내용은 그 이전 구조의 기록으로 남겨두고, 현재 실제 동작하는 구조는 3-11을 참고할 것.
...
## 3-11. 아이돌/드라마 매치에 relationshipMode(친구/룸메이트/그룹케미) 추가 — 로맨틱 프레이밍 완전 제거

3-9에서 만든 "bias match" 읽기(팬-최애 감정 언어, `Twin Flame`/`Magnetic Tension` 같은 티어명)를 **로맨틱 프레이밍이 전혀 없는 3가지 렌즈**로 교체함 — 궁합 계산 자체(`getCompatibility`/`getCompatibilityScore`, 관계 5종 판정)는 완전히 그대로 재사용하고, 그 결과를 어떤 콘텐츠로 보여줄지만 유저가 선택하는 구조. 결과 화면 상단 탭 3개(친구/룸메이트/그룹케미, 기본값 `친구`)로 전환하면 점수·오행 배지는 그대로 둔 채 인사이트 섹션 전체·티어·공유카드 문구가 즉시 다시 계산됨(리로드 없음, `findBestMatch` 재실행도 없음 — relationshipMode는 순수 표시 레이어).

- **신규 콘텐츠 뱅크 3개**(`friendshipTemplates.js`/`roommateTemplates.js`/`groupChemistryTemplates.js`) — 관계 5종(`same`/`otherGeneratesMe`/`iGenerateOther`/`otherOvercomesMe`/`iOvercomeOther`) × en/ko로 전부 새로 작성, 아이돌매치·드라마매치 공용(배우/아이돌 구분 없이 하나의 뱅크를 같이 씀 — meetingScenario 때와 달리 이 콘텐츠는 "내 일상"(여행/카페/자취) 기준이라 대상이 아이돌이든 배우든 안 갈릴 이유가 없음). 세 뱅크 다 관계마다 `tier`(짧은 라벨)+`line`(한 줄)+`explanation`+`watchFor`, 그리고 모드별 섹션(친구: `travelStyle`/`cafeChemistry`/`howTheySeeYou` 3개, 룸메이트: `livingPattern`/`conflictStyle`/`dailyMoment` 3개, 그룹케미: `stagePresence`/`offstage` **2개만** — 무대 위/무대 밖이라는 대비 쌍이라 3개로 안 늘림, 사용자 스펙 3번 항목이 명시한 필드 그대로)를 `{ subheading, text }`로 담음. 문체는 기존 원칙 그대로(구체적 장면 묘사 + "~했던 적 있을 거예요" 반문형 회상 + "반대로" 대조 구조, 불안 유발형 금지)
- **그룹케미 티어는 사용자가 직접 지정한 매핑 그대로 사용**: `same`→찐친/Ride-or-Die Besties, `otherGeneratesMe`→서로 의지하는 관계/The Ones Who Lean On Each Other, `iGenerateOther`→맏언니·막내 케미/Big Sibling Little Sibling Energy(성별 무관하게 "손위-손아래" 포지션으로 해석, 실제 두 사람의 성별과 무관), `otherOvercomesMe`→티격태격 콤비/The Bickering Duo, `iOvercomeOther`→무대 위 최강 듀오/The Strongest Duo On Stage. 친구/룸메이트 모드의 티어명은 스펙에 명시 안 돼서 같은 관계-플레이버 컨벤션(same=완벽 동기화, otherGeneratesMe=편안함을 받음, iGenerateOther=내가 먼저 챙김, otherOvercomesMe=예측불가 긴장감, iOvercomeOther=내가 이끎/지켜줌)에 맞춰 새로 작명함(예: "텔레파시 단짝", "생활 패턴 완벽 일치")
- **`IdolMatch.jsx`/`DramaMatch.jsx` 공통 `MODE_CONFIG` 디스패치 테이블**: `{ friend: {getCopy: getFriendshipCopy, sections:[...]}, roommate: {...}, groupChemistry: {...} }` — `buildInsightSections(t, modeKey, modeCopy)` 헬퍼 하나가 `[explanation(locked:false), ...모드별 섹션(locked:true), watchFor(locked:true)]`를 조립(그룹케미만 4개, 나머지는 5개). 기존 `getIdolMatchCopy`/`getDramaMatchCopy` 호출, `pillarSituational`/`getPillarCompatibility` 기반 기둥별 섹션, `meetingScenarioTitle` i18n 키는 전부 이 페이지들에서 제거됨(3-9 참고 — 대체됨 표시해둠)
- **탭 UI**: 두 페이지 다 결과 카드 바로 위(제목/서브타이틀 아래)에 `role="tablist"` 버튼 3개 — `useState('friend')`로 기본값 고정, 클릭 시 `relationshipMode` state만 바뀌고 리렌더링. 아이돌매치는 베스트매치 화면과 그룹모드(멤버 직접 선택) 결과 화면 둘 다에 동일하게 배치
- **공유카드 데이터 소스 전환**: `IdolShareCard`에 넘기던 `tier`/`subheading`/`text`가 (기존 `compatCopy.meetingScenario` 대신) 새 `compatCopy.tier`/`compatCopy.explanation.{subheading,text}`로 바뀜 — 화면에 보이는 관계 프레이밍과 공유 이미지가 항상 일치하도록(다운로드 직전에 모드를 바꿨는데 공유카드만 옛날 로맨틱 문구로 나가는 불일치 방지)
- **`idolMatchTemplates.js`/`dramaMatchTemplates.js` 처리**: 사용자 지시가 "궁합 계산 엔진은 재사용, 콘텐츠만 신규"였고 결과 화면의 섹션 구성 자체가 통째로 바뀌는 구조라, 두 파일(`tier`/`line`/`goodFit`/`meetingScenario`/`pillarSituational`)이 이 두 페이지에서 완전히 미사용 상태가 됨 — 대신 삭제하지 않고 그대로 남겨둠(상당한 콘텐츠 투자였던 데다, 삭제는 되돌리기 어려운 결정이라 이번 스코프에서 판단하지 않기로 함). 필요 없다고 확인되면 다음에 삭제 가능
- **검증**: Playwright로 (1) idol-match/drama-match × en/ko × 3개 모드 전부 조합(12가지)에서 섹션 개수가 스펙대로(친구/룸메이트 5개, 그룹케미 4개) 나오는지, 섹션 제목이 정확한지, 본문에 `undefined`가 섞이지 않는지 확인 — 전부 일치 (2) DOM 블러 검사로 explanation 섹션만 제목+본문 둘 다 안 블러되고, 나머지 섹션은 제목은 안 블러·본문만 블러되는지 확인 — 정확히 일치 (3) `friendshipTemplates.js`/`roommateTemplates.js`/`groupChemistryTemplates.js` 전체 텍스트를 정규식으로 스캔해서 금지 키워드(couple/partner/boyfriend/girlfriend/커플/연인/사귀/결혼/연애) 0건 확인 (4) 아이돌매치 그룹모드(멤버 직접 선택) 결과 화면에서도 탭이 정상 동작하고 그룹케미 티어("Big Sibling, Little Sibling Energy" 등)가 정확히 뜨는지 스크린샷으로 확인 — 콘솔 에러 0건

## 3-12. relationshipMode 가이드 보강 — watchFor 톤 수정, 그룹케미 매핑 근거 명시, 십성/귀인 기반 신규 섹션

3-11에서 만든 친구/룸메이트/그룹케미 3종 콘텐츠 뱅크에 세 가지를 추가로 보강함.

- **1. `watchFor` 톤을 "조심하세요"에서 "그래도 가끔은 ~하는 것도 재밌을 거예요"로 전면 재작성**: 3개 뱅크(friendship/roommate/groupChemistry) × 관계 5종 × en/ko = 30개 `watchFor` 항목 전부, "필요해요"/"조심하세요"/"~하는 게 좋아요" 같은 지적·교정형 문장을 "~해보는 것도 재밌을 거예요"/"~해보는 게 좋은 반전이 될 거예요" 같은 긍정 제안형으로 다시 씀. 내용의 핵심(예: same 관계는 "너무 잘 맞아서 새로운 자극이 필요함")은 그대로 유지하고, 프레이밍만 "문제를 짚어주는 어조"에서 "재밌는 걸 제안하는 어조"로 바꿈
- **2. 그룹케미 관계→케미유형 매핑 근거를 `groupChemistryTemplates.js` 상단 주석으로 명시**: `same`(오행이 같아 이미 통함)/`otherGeneratesMe`(상대 기운이 나를 채워줌 = 기댐)/`iGenerateOther`(내가 상대를 채워줌 = 맏언니·막내)/`otherOvercomesMe`(상대가 나를 극함 = 실제 마찰이지만 매력적인 텐션)/`iOvercomeOther`(내가 상대를 극함 = 내가 방향을 잡는 리더십) — 사용자가 준 근거를 그대로 코드 주석화해서, 왜 이 5개 매핑인지 다음에 봐도 바로 이해되게 함
- **3. 십성/신강신약 기반 신규 섹션 2개 추가 — "우리 둘의 케미 포인트"(상시) + "귀인 케미"(조건부)**: 기존 오행 관계 5종과는 완전히 독립된 축을 하나 더 얹음. `getTenGodProfile(saju)`(이미 3-1에 있던 함수, 새 계산 없음)로 유저와 상대 각각의 최다 십성 카테고리(비겁/식상/재성/관성/인성)를 구해서, 같으면 "같은 결의 매력을 가진 둘"(카테고리별 5개 변형), 다르면 "서로 다른 매력이 만나는 조합"(공용 1개)으로 분기 — 오행 관계가 같아도 십성 조합이 다르면 실제로 다른 문구가 나오는 구조. `getNobleman(saju, lang)`(3-1) 결과 유저 또는 상대 어느 한쪽이라도 `hasNobleman`이면 "귀인 케미" 섹션이 추가로 등장(없으면 섹션 자체가 안 뜸 — 배열 길이가 6~7개로 가변적).
  - **7개 콘텐츠 뱅크 전부에 추가**: 3-11에서 만든 3개(friendship/roommate/groupChemistry) + 실제 페이지에 연결된 2개(compatibilityTemplates.js/romanceTemplates.js는 이번 스코프 아님, 사용자가 "아이돌매치/드라마매치"로 문제를 한정했기 때문에 제외) 대신, **idolMatchTemplates.js/dramaMatchTemplates.js**(3-11에서 이미 미사용 상태가 된 레거시 파일)까지 포함해서 정확히 5개 파일에 추가함 — 사용자 메시지의 "총 7개" 카운트는 "explanation/goodFit/meetingScenario/기둥별/watchFor"(레거시 2파일의 필드 종류)와 "친구·룸메이트·그룹케미"를 합친 숫자로 재구성한 해석. **이 해석에 확신이 없어서 여기 명시적으로 남김** — 만약 compatibilityTemplates.js/romanceTemplates.js(Compatibility.jsx/Romance.jsx)에도 이 기능이 필요하면 후속 작업으로 진행 가능. `idolMatchTemplates.js`/`dramaMatchTemplates.js`에 추가한 콘텐츠는 파일 상단 NOTE 주석대로 여전히 어느 페이지에서도 안 쓰임(레거시 상태 유지, 일관성을 위해 추가만 해둠)
  - **각 파일에 `chemistryPoints`(관계 키와 별개로 언어 객체 최상위에 위치) + `noblemanBonus`** 신규 데이터, 그리고 `getChemistryPoints(lang, myCategory, otherCategory)`/`getNoblemanBonus(lang)` 신규 getter 2개씩 추가(5파일 × 2 = 10개 함수, 이름은 전부 동일하되 각 파일에서 개별 export이므로 `IdolMatch.jsx`/`DramaMatch.jsx`에서 import 시 `as getFriendChemistryPoints` 식으로 별칭 처리)
  - **`IdolMatch.jsx`/`DramaMatch.jsx`의 `buildInsightSections` 시그니처 확장**: `(t, modeKey, modeCopy)` → `(t, lang, modeKey, modeCopy, { myTenGod, otherTenGod, hasNobleman })`. 섹션 순서가 `[explanation(무료), 케미포인트(잠금), 귀인케미(잠금, 조건부), ...모드별 섹션(잠금), watchFor(잠금)]`로 변경 — 나머지 6-7개 잠금 구조(제목 항상 보임/본문만 블러)는 기존 `InsightSection`/`PremiumLock` 그대로라 컴포넌트 수정 없음. `myTenGod`/`otherTenGod`/`hasNobleman`은 두 곳(베스트매치 `best`/그룹모드 `memberCompat`)에서 각각 `getTenGodProfile`/`getNobleman`을 유저·상대 saju에 직접 호출해서 계산
  - 새 i18n 키: `matchCommon.insightTitles.{chemistryPoints, noblemanBonus}` (en/ko)
- **검증**: (1) 서로 다른 생년월일 5개로 베스트매치를 돌려서 섹션 개수가 6개(귀인 없음) 또는 7개(귀인 있음)로 정확히 갈리는지, "Our Chemistry Point"/"우리 둘의 케미 포인트" 서브헤딩이 실제로 존재하는지 확인 — 5개 전부 우연히 "다른 카테고리"(different) 분기만 나와서, 그룹모드(멤버 직접 선택)로 BTS RM을 골라 "같은 카테고리"(same) 분기도 실제로 다른 문구로 렌더링되는지 별도 확인(스크린샷) — 두 분기 모두 정상 작동 확인 (2) 귀인 케미 섹션이 있을 때만 나타나고 없을 때는 배열에서 통째로 빠지는지 확인 (3) DOM 블러 검사로 새 섹션 2개도 기존 원칙(제목 항상 보임, 본문만 블러) 그대로 따르는지 확인 — 정확히 일치 (4) en/ko 둘 다 확인, DramaMatch.jsx에서도 동일하게 동작 확인 — 콘솔 에러 0건

## 3-13. `chemistryPoints`를 이진법→25매트릭스로 확장 + `/compatibility`에 일주 오행 개인화 추가

두 갈래 작업.

- **1. `getChemistryPoints(lang, myCategory, otherCategory)` — same/different 이진법을 십성 5대분류 × 5대분류 = 25칸 매트릭스로 전면 재구성**: 3-12에서 추가한 `chemistryPoints` 뱅크(5개 파일: friendship/roommate/groupChemistry/idolMatch/dramaMatch)를 `{same:{...}, different:{subheading,text}}`에서 `{companion:{companion,output,wealth,officer,resource}, output:{...}, ...}` 형태의 완전한 5×5로 재작성(en/ko, 5파일 전부). 대각선 5칸(같은 카테고리끼리)은 기존 "같은 매력" 텍스트를 그대로 유지. 비대각선 20칸은 **10개의 순서 없는 카테고리 쌍**(비겁-식상/비겁-재성/비겁-관성/비겁-인성/식상-재성/식상-관성/식상-인성/재성-관성/재성-인성/관성-인성)마다 새 텍스트를 하나씩 써서, 두 매트릭스 방향(예: `companion.output`과 `output.companion`)에 동일하게 재사용 — 사용자가 준 예시("비겁×재성 = 대등하게 통하는 사람과 챙겨주는 사람의 만남")가 방향을 구분하지 않는 서술이었기 때문에 내린 판단. 25칸 전부 채워져 있어 구조적으로는 "25개 조합"을 만족하지만, 실제 고유 문구는 파일당 15개(대각선 5 + 쌍 10)임 — **이 설계 판단은 사용자에게 명시적으로 확인받지 않았으니 이견 있으면 알려달라고 안내 필요**. `getChemistryPoints` 함수 시그니처(`(lang, myCategory, otherCategory)`)는 그대로, 반환값(bank 룩업 결과)만 25종으로 확장. Node 스크립트로 5파일×2언어 전부 25/25 칸 채워짐을 구조적으로 검증(빌드는 통과해도 매트릭스 칸 누락은 런타임에서만 터지는 문제라 별도 검증함).
- **2. `/compatibility`(궁합보기) 전용 — `explanation`/`goodFit`/`watchFor`에 일주(日柱) 오행 개인화 추가**: `compatibilityTemplates.js`에 새 `explanation`(관계 5종, en/ko — 이 파일에 원래 없던 필드라 새로 작성, "왜 이 점수인지"를 설명하는 톤은 `matchCommon.explanation`과 유사하되 문구는 전부 새로 씀) + `dayGanFlavor`(일주 오행 5종 × `{explanation, goodFit, watchFor}` 각 1문단, en/ko) 두 뱅크를 추가. `getCompatibilityCopy(lang, relation, seedInput, dayGanElement)`에 4번째 파라미터(선택) 추가 — 있으면 관계 5종 베이스 텍스트 뒤에 해당 오행의 `dayGanFlavor` 문단을 이어붙이고, 없으면(호출부가 안 넘기면) 기존처럼 관계 5종 텍스트만 반환(하위호환). `Compatibility.jsx`에서 `mySaju.dayGanElement`를 4번째 인자로 넘기도록 수정 — 이제 같은 관계값(예: same)이어도 내 일주가 목/화/토/금/수 중 뭐냐에 따라 explanation/goodFit/watchFor 세 섹션 전부 다른 문구가 뜸(관계 5 × 일주 5 = 25 조합).
  - **`explanation` 소스를 `matchCommon.explanation`(i18n JSON, Romance.jsx와 공유)에서 `compatibilityTemplates.js`의 새 필드로 전환**: 원래 `Compatibility.jsx`는 `t('matchCommon.explanation.${relation}...')`을 직접 읽었는데, 이건 Romance.jsx도 같이 쓰는 공유 콘텐츠라 여기에 일주 개인화를 얹으면 Romance.jsx까지 영향을 받게 됨. 그래서 `matchCommon.explanation`은 완전히 그대로 두고(Romance.jsx 무영향), `/compatibility` 전용으로 `compatibilityTemplates.js` 안에 독립된 `explanation` 필드를 새로 만들어 `getCompatibilityCopy`가 반환하도록 전환 — `IdolMatch.jsx`/`DramaMatch.jsx`는애초에 `matchCommon.explanation`을 안 씀(무관).
...
- **`explanation` 필드는 `getIdolMatchCopy`/`getDramaMatchCopy` 자체가 반환하도록 수정(최초 구현에서는 페이지 컴포넌트 쪽에서 `{...copy, explanation}`으로 나중에 합성했었는데, 후속 지적을 받고 이 방식으로 다시 고침 — 아래 3-15-1 참고)**: 두 함수 모두 새 파라미터 `myElement`/`otherElement`를 받아, 신규 헬퍼 `buildExplanation(lang, relation, myElement, otherElement)`가 `matchCommon.explanation.{relation}`(공유 i18n, Compatibility.jsx/Romance.jsx도 씀)을 **`t()` 없이 `en.json`/`ko.json`을 직접 import**해서 읽고 `{{my}}`/`{{other}}`를 정규식으로 직접 보간해서 채워 넣음(이 함수들은 React 컴포넌트가 아니라 순수 데이터 함수라 `t()`에 접근할 수 없음). 결과적으로 `getIdolMatchCopy`/`getDramaMatchCopy`의 반환값 모양이 나머지 3개 모드(`getFriendshipCopy`/`getRoommateCopy`/`getGroupChemistryCopy`)와 완전히 동일한 `{tier, line, explanation, goodFit, ...}` 구조가 됨 — `IdolMatch.jsx`/`DramaMatch.jsx`는 이제 `relationshipMode === 'compatibility'`일 때도 그냥 `getIdolMatchCopy(...)`/`getDramaMatchCopy(...)`를 직접 호출하기만 하면 되고, 호출부에서 `explanation`을 따로 조립하는 코드는 완전히 제거됨.
...
### 3-15-1. 후속 수정 — explanation을 페이지가 아니라 데이터 함수가 직접 반환하도록 리팩터(공유카드 검증 포함)

3-15에서 처음 구현할 때는 `IdolMatch.jsx`/`DramaMatch.jsx`가 `getIdolMatchCopy`/`getDramaMatchCopy` 호출 결과에 `explanation`을 스프레드로 덧붙이는 방식이었음(`{ ...getIdolMatchCopy(...), explanation: {...} }`). 기능적으로는 그 시점에도 `memberCopy.explanation`이 항상 채워져 있었지만(IdolShareCard.jsx의 `text`/`subheading` prop이 `text ? ... : ''`/`{subheading && ...}`로 방어 코드가 있어서 실제로 undefined 크래시가 나진 않았을 것으로 보임), 사용자가 "getIdolMatchCopy/getDramaMatchCopy 자체가 explanation을 반환하도록" 명시적으로 재작업을 요청해서 위 방식으로 리팩터함 — 데이터 함수의 반환 모양이 4개 모드 전부 동일해지는 게 더 안전하고 일관적인 설계이기도 함.

**공유카드 실사용 검증(사용자가 명시적으로 요구한 검증 방식)**: Playwright로 헤드리스 브라우저에서 실제로 "이미지로 저장" 버튼을 클릭해서 `useShareCardDownload.js`의 `saveImage`(`html-to-image`의 `toPng()` 호출) → 실제 다운로드 이벤트까지 전체 플로우를 en/ko 양쪽에서 재현 — 궁합/친구/룸메이트/그룹케미 4개 모드 전부(아이돌매치 솔메이트+그룹모드, 드라마매치 포함 총 7개 조합)에서 다운로드가 실제로 발생하는지(`page.waitForEvent('download')`), 오프스크린 카드의 렌더 텍스트가 비어있지 않은지 확인. 특히 궁합 모드에서 실제 카드 텍스트를 직접 출력해서 확인 — 헤드라인("Two People Speaking One Language"/"같은 언어를 쓰는 두 사람")과 `{{my}}`/`{{other}}` 보간(예: "Earth and Earth"/"토 오행과 토 오행")이 정확히 채워지는 것을 육안으로 확인함. 콘솔 에러 0건, 7개 조합 전부 정상.

## 3-16. 랜딩 페이지 섹션 묶음 이름 변경 (`home.readingsLabel`/`home.idolsLabel`)

`Landing.jsx`의 3개 섹션 라벨 중 2개를 en/ko 둘 다 교체 — `home.readingsLabel`: "Saju Readings"/"사주 리딩" → "My Ohaeng"/"내 오행 보기", `home.idolsLabel`: "K-Pop & K-Drama"/"K팝 & K-드라마" → "Fandom Match"/"덕질 매치". `home.romanceLabel`("Romance"/"연애")은 그대로. 섹션 안 개별 메뉴 항목명(오늘의 운세/내 사주/궁합, 최애매치/아이돌매치/드라마매치 등)은 전혀 안 건드림 — i18n 키 2개만 값 교체, `Landing.jsx` 자체는 무수정. 영문 "My Oheng"이라는 사용자 표기는 앱 브랜드명 표기(`app.name` = "Ohaeng")에 맞춰 "My Ohaeng"으로 고쳐서 적용함. Playwright로 랜딩 페이지 `.menu-section-label` 텍스트를 en/ko 둘 다 확인, 섹션 안 개별 항목 텍스트는 이전과 동일한지도 전체 텍스트 덤프로 대조. 콘솔 에러 0건.
...
## 5-7. 영문 콘텐츠의 "Compare that to a X" 대조 구문 반복 제거

`friendshipTemplates.js`/`roommateTemplates.js`(en만 — ko는 애초에 이 패턴이 없어서 안 건드림)의 `travelStyle`/`cafeChemistry`/`dailyMoment` 섹션에서 "Compare that to a [X], and..." 형태의 문장이 관계값 5종 중 여러 곳에서 똑같은 수사적 틀로 반복되고 있던 걸 발견 — `friendshipTemplates.js`의 `travelStyle`은 관계 5종 **전부**, `cafeChemistry`는 5종 중 4종이 이 패턴이었음(9곳), `roommateTemplates.js`엔 1곳. 콘텐츠(사실관계·톤)는 그대로 두고 마지막 문장(대조를 표현하는 부분)만 관계값마다 다른 수사 장치로 재작성함 — 직설적 단언, 반문, 비유("보이지 않는 접착제"), 2인칭 가정("상상해보면"), 장면 묘사(대안을 직접 언급하지 않고 결과만 보여주기), 절제된 한 줄 마무리, 희소성 진술, 회고형 진술 등 9가지를 서로 다르게 배치해서 같은 뱅크 안에서 관계값을 나란히 비교해도 문장 구조가 안 겹치게 함. `groupChemistryTemplates.js`/`idolMatchTemplates.js`/`dramaMatchTemplates.js`는 애초에 이 패턴이 없어서(검색 결과 0건) 안 건드림. 헤드리스 브라우저로 아이돌매치(솔메이트+친구/룸메이트 탭)·드라마매치 렌더 텍스트에 "Compare"/"Compared" 문자열이 더 이상 없는지 확인, 콘솔 에러 0건.
...
## 7-6. 오행 아이콘(`src/assets/icons/elements/*.png`) 얼굴 크롭 가로 중심 로직 수정

7-2에서 "아이콘 얼굴 크롭 작업"으로 처음 만들어진 5개 아이콘(캐릭터 전신 PNG에서 얼굴만 잘라낸 원형 배지용 이미지)이, 가로 중심을 몸통 전체(스크롤 등 옆으로 뻗은 소품 포함)의 alpha bbox 기준으로 잡고 있어서 소품이 있는 쪽으로 크롭 중심이 쏠리는 문제가 있었음 — 특히 wood(청룡)가 손에 든 두루마리가 오른쪽으로 크게 뻗어 있어서 얼굴이 왼쪽으로 심하게 치우쳐 보였음.

- **원본 크롭 스크립트는 레포에 안 남아있어서(생성 후 삭제된 임시 스크립트로 추정) 기존 아이콘 크기를 역산해서 로직을 재구성**: `src/assets/characters/*.png`(500×500) 각각의 전신 alpha bbox를 계산해보니, 세로 크롭 크기(정사각형, 한 변 = 대략 `전신bbox높이 × 0.65`)가 기존 아이콘 파일 크기(274~281px)와 거의 일치함을 확인 — 이 세로 크롭 공식(높이 65%, 크롭 시작점을 bbox 상단에서 위로 12%만큼 올림)은 그대로 유지.
- **가로 중심만 교체**: 전신 bbox 대신, **전신 bbox의 상단 40%에 해당하는 행(row) 구간만 잘라서 그 구간의 alpha bbox를 별도로 계산**(= "머리만 있는 영역") → 그 bbox의 가로 중심에 정사각형 크롭을 맞춤. 소품은 대부분 손/허리 높이(bbox 하단 60%)에 있어서 상단 40%만 보면 자연스럽게 제외됨. 실제로 wood의 전신 bbox는 가로 79~498(중심 288.5, 두루마리 때문에 오른쪽으로 심하게 치우침)인데 상단 40% 헤드 영역만 보면 82~406(중심 244)로, earth(같은 몸 형태, 소품 없음)의 헤드 중심 244와 정확히 일치 — 로직이 의도대로 작동함을 수치로 확인.
- **구현**: `sharp`(이미 `workers-og`의 의존성으로 설치돼 있어서 별도 설치 불필요)로 각 PNG의 raw 픽셀 버퍼를 읽어 alpha 채널을 직접 스캔하는 임시 Node 스크립트 작성 → 5개 아이콘 재생성 → `src/assets/icons/elements/`에 같은 파일명으로 덮어쓰기(스크립트 자체는 검증 후 삭제, import 구조라 이 5개 PNG를 쓰는 헤더 로고/`ElementBadge`/공유카드 등은 전부 코드 수정 없이 자동으로 새 크롭을 반영).
- **검증**: 교체 전 아이콘을 임시 백업해서 5개 전부 육안 비교 — wood는 얼굴이 원 중앙으로 확실히 이동(뿔·수염이 양쪽 균형 있게 보임), 나머지 4개(fire/earth/metal/water)도 손에 든 소품이 사라지고 중앙 정렬 유지. `/guide` 페이지의 실제 `ElementBadge` 원형 배지(5개 전부 노출되는 페이지)를 Playwright로 렌더링해서 실제 서비스 화면에서도 잘 보이는지 확인, 콘솔 에러 0건.
...
- **프리미엄 잠금 — 두 겹 구조(5-4, 5-6 참고)**: (1) `src/config.js`의 `PREVIEW_MODE_UNLOCK_ALL` — **현재 `false`**, 개발자가 전체 리뷰할 때만 잠깐 `true`로 켰다가 다시 꺼두는 용도(이제 `PremiumLock`/`CreditLock` 둘 다 이 플래그를 봄). (2) `PremiumContext`/`utils/premiumUnlock.js` — 방문자가 잠긴 섹션의 "이 콘텐츠 무료로 체험하기" 버튼을 눌러 localStorage(`premiumUnlockedProducts`, 상품별 JSON 객체)에 저장하는 **실제 라이브 잠금 해제 흐름**. **13번(Paddle 크레딧) 이후로는 `saju`/`compatibility`/`romance` 3개만 이 방식**이고, `idolMatch`/`dramaMatch`는 완전히 별도의 크레딧 시스템(서버 D1 저장, 13번 참고)으로 전환됨 — `PRODUCTS` 배열에서도 그 둘을 뺐음
...
- **수익화 — 아이돌매치/드라마매치는 Paddle 크레딧으로 절반 완료**(13번 참고). 코드는 전부 작성·로컬 단위테스트 완료. **막힌 부분**: 사용자가 Paddle 계정 생성 + API 키 발급, `wrangler login`, Cloudflare D1 바인딩 — 전부 계정/대시보드 작업이라 대신 못 함. saju/compatibility/romance 3개는 여전히 Stripe 미연동 상태(`utils/premiumUnlock.js`의 `setProductUnlocked(productKey, true)`만 호출하면 되도록 준비는 돼있음, 5-6 참고)
...
## 13. Paddle 크레딧 결제 시스템 — 실계정 연동·실사용 결제 테스트까지 완료 (13-1/13-2 참고, saju/compatibility/romance도 이후 편입됨)

이 프로젝트 **최초의 서버 저장소(Cloudflare D1)이자 최초의 실제 결제 연동(Paddle)**. 상세 설계 배경·아키텍처 다이어그램은 플랜 파일(`quirky-greeting-pudding.md`, 세션 재개 시 참고 가능)에 더 자세히 있음 — 여기는 요약.

- **범위를 처음엔 의도적으로 좁힘 (이후 13-2에서 전체로 확장됨)**: saju/compatibility/romance 3개는 기존 `PremiumContext`(상품 전체를 한 번에 영구 언락) 그대로 두고, **idolMatch/dramaMatch 2개만 크레딧 소비형으로 전환** — 크레딧 1개 = 특정 멤버/배우 한 명(예: `idol:bts-rm`, `actor:song-joongki`)의 전체 리딩 영구 언락. `utils/premiumUnlock.js`의 `PRODUCTS`에서 `idolMatch`/`dramaMatch` 제거함(Guide.jsx 재잠금 테스트 패널도 자동으로 3개만 보임). **13-2에서 나머지 3개도 크레딧 시스템으로 옮겨서, 지금은 5개 전부 크레딧 기반 — `PremiumContext`/`premiumUnlock.js`는 파일만 남고 실제로 쓰는 곳은 없음.**
- **왜 서버 저장소가 필요했는지**: Paddle 웹훅은 Cloudflare Pages Function(서버)에서 도는데, 이 앱은 로그인/계정이 없어서 서버가 "이 결제를 누구 브라우저에 반영할지" 알 방법이 localStorage뿐인 기존 구조로는 없음(서버가 특정 브라우저의 localStorage를 못 건드림). 그래서 **Cloudflare D1**을 처음 도입 — `credits`(잔액)/`unlocks`(멤버별 언락)/`processed_transactions`(웹훅 중복 방지) 3테이블, 스키마는 `db/schema.sql`.
- **익명 유저 식별**: 로그인 없이 `crypto.randomUUID()`를 브라우저 localStorage(`ohaeng_anon_id`, `src/utils/credits.js`의 `getOrCreateAnonId()`)에 저장해서 "이 브라우저"를 구분. UUID 자체가 추측 불가능한 값이라는 걸 사실상의 인증 수단으로 씀(계정 시스템 도입 전 임시 설계로 사용자와 합의됨) — anonId가 유출되는 새 경로가 이번 코드에 없는지는 보안 리뷰로 확인함.
- **크레딧 지급 경로는 딱 둘**: ① `functions/webhooks/paddle.js` — Paddle 결제 완료(`transaction.completed`) 웹훅, `Paddle-Signature` 헤더를 HMAC-SHA256으로 검증(`functions/_lib/paddleSignature.js`, developer.paddle.com 공식 스펙대로 구현)한 뒤에만 크레딧 지급. `processed_transactions`에 트랜잭션ID를 PK로 먼저 INSERT해서 웹훅 재시도로 인한 중복 지급을 막음. ② `functions/api/credits/state.js` — anonId의 **첫 조회**에만 웰컴 크레딧 1개를 서버에서 판단해 지급(devtools로 조작 불가). 클라이언트의 Paddle.js `checkout.completed` 이벤트는 잔액 폴링용 UI 트리거일 뿐, 그 자체로 크레딧을 늘리지 않음.
- **소비**: `functions/api/credits/consume.js` — **`unlocks` INSERT를 먼저 하고 잔액 차감을 나중에** 하는 순서(처음엔 반대로 짰다가 보안 리뷰에서 지적받고 순서를 바꿈, 아래 참고). `unlocks`의 PK(`anon_id`+`item_key`)가 실제 동시성 가드 역할을 함 — 같은 아이템에 대한 동시 요청은 하나만 INSERT 성공하고 나머지는 제약 위반으로 실패해서 `alreadyOwned`로 처리, 잔액은 안 건드림. INSERT는 성공했는데 잔액이 이미 0이면 방금 넣은 unlocks 행을 롤백(DELETE)해서 공짜 언락이 안 생기게 함. *(남은 아주 좁은 한계: 같은 anonId가 서로 다른 두 아이템을 초저지연으로 동시에 소비하면 이론상 1크레딧 초과 소비 가능 — 개인 프로젝트 규모에서 감수, 강화하려면 Durable Object로 승격)*
  - **보안 리뷰에서 실제로 잡힌 버그**: 독립 서브에이전트로 돌린 `security-review`에서 Medium(신뢰도 7/10) 발견 — 원래 코드는 "이미 언락됐나 확인 → 잔액 차감 → unlocks에 기록" 순서였는데, 같은 아이템을 동시에 두 번 요청하면(더블클릭 등) 둘 다 "언락 안 됨" 체크를 통과하고 둘 다 잔액을 차감한 뒤, 두 번째 INSERT가 PK 충돌로 실패(그런데 try/catch가 없어서 처리 안 된 에러로 튐) — 결과적으로 크레딧 1개짜리 언락에 2개가 빠져나가는 버그였음. 위 순서로 재배치해서 고치고, `node:sqlite` 기반 유닛테스트로 "동시에 같은 아이템 두 번 요청 → 잔액이 정확히 1만 줄어드는지" 직접 재현·확인함.
- **프론트엔드 연결부**: `InsightSection.jsx`에 `unlockKey` prop 추가 — 있으면 잠긴 섹션을 `PremiumLock` 대신 `CreditLock.jsx`(같은 블러+오버레이 UI, 크레딧 있으면 "1개로 보기" 버튼, 없으면 `/credits`로 링크)로 감쌈. `MatchResultCard.jsx`는 `insightUnlockKey`로 그대로 통과. `IdolMatch.jsx`/`DramaMatch.jsx`는 결과 화면 상단에 "Credits: N · Buy credits" 표시(`useCredits()` 훅, `CreditsContext.jsx` — `PremiumContext.jsx`와 동일 패턴이되 서버에서 상태를 가져옴) 추가하고, `insightProduct` 대신 `insightUnlockKey={idolItemKey(memberId)}`/`actorItemKey(actorId)` 전달.
- **구매 플로우**: `/credits`(`Credits.jsx`, 신규 라우트) — 3개 팩 버튼($0.99/1개, $2.49/3개, $3.99/5개, `src/data/creditPacks.js`), `@paddle/paddle-js`(Paddle 공식 npm 패키지)로 오버레이 체크아웃을 염. `VITE_PADDLE_CLIENT_TOKEN` 미설정 시("아직 준비 중" 문구로) 크래시 없이 안전하게 폴백 — 이번 세션엔 이 값이 없어서 실제로 이 폴백 경로로 렌더되는 것까지 확인함.
- **Paddle 계정 설정 자동화**: `scripts/setup-paddle.mjs` — Paddle API로 상품 1개("Ohaeng Credit Pack") + 가격 3개 생성, 웹훅 목적지 등록까지 자동화하고 `functions/_lib/priceCredits.js`/`src/data/creditPacks.js`의 플레이스홀더 가격ID를 실제 값으로 자동 치환함. **웹훅 시크릿은 터미널에 출력하지 않고 `.env.local`에 직접 append**(보안 정책상 API 키/시크릿은 채팅에 노출 안 시키는 원칙을 지키기 위한 설계) — 사용자가 그 파일을 직접 열어서 Cloudflare에 옮겨야 함.
- **검증**: `node:sqlite`(Node 22+ 내장, 실험적)로 D1 API를 흉내 낸 인메모리 shim을 만들어서 `db/schema.sql`을 실제로 실행하고, 4개 서버 함수 전부를 실제 요청 객체로 직접 호출하는 단위 테스트 작성(웰컴 크레딧 1회만 지급, 소비 성공/이미소유/잔액부족(402)/잘못된 itemKey(400)/동시 요청 시 정확히 1회만 차감, 웹훅 정상 지급/재전송시 중복지급 안됨/서명 위조 거부/틀린 시크릿 거부 — 총 15개, 전부 통과) — Cloudflare 인프라 없이도 로직 정확성을 검증한 방법. 프론트엔드는 Playwright로 API가 아직 연결 안 된 상태(로컬 `vite dev`, D1/Paddle 미설정)에서도 크래시 없이 "Credits: 0 · Buy credits"로 우아하게 폴백하는지 확인, 콘솔 에러 0건. **`security-review` 스킬(독립 서브에이전트)로 새 코드 전체 검토 — Medium 발견 1건(위 소비 로직 레이스) 실제로 고침**, 그 외 SQL 인젝션/서명 검증 우회/시크릿 노출/IDOR 관점에서는 신뢰도 7 이상 발견 없음.
- **사용자가 직접 해야 할 일 (제가 못 하는 부분)**: (1) Paddle 계정 생성 + API 키/클라이언트 토큰 발급 → API 키는 로컬 `.env.local`에 `PADDLE_API_KEY=`로(채팅에 붙여넣지 않기) (2) `.env.local`에 값 채운 뒤 `node scripts/setup-paddle.mjs https://getohaeng.com/webhooks/paddle` 실행(제가 대신 실행 가능) (3) `npx wrangler login`으로 Cloudflare 인증(이 환경은 현재 미인증 확인함) → 이후 `wrangler d1 create ohaeng-credits`는 제가 대신 실행 가능 (4) **Cloudflare Pages 대시보드**에서 D1 바인딩(변수명 `DB`)과 환경변수(`PADDLE_WEBHOOK_SECRET` Secret, `VITE_PADDLE_CLIENT_TOKEN`, `VITE_PADDLE_ENV=sandbox`) 등록 — 이 프로젝트는 Git 연동 자동배포라 wrangler.toml이 안 읽혀서 대시보드에서 직접 해야 함. **(전부 완료됨 — 아래 13-1 참고.)**

## 13-1. Paddle 실계정 연동 완료 + 실제 샌드박스 결제로 전체 플로우 검증

위 "사용자가 직접 해야 할 일" 4가지가 실제로 전부 처리됨 — `wrangler login`(OAuth, 첫 시도는 타임아웃, 재시도로 성공), `wrangler d1 create ohaeng-credits`(APAC 리전) + `db/schema.sql` 원격 적용, Cloudflare Pages 대시보드에 D1 바인딩(`DB`)과 5개 환경변수(Production+Preview) 등록까지 사용자가 완료. 그 과정에서 **로컬 mock/유닛테스트로는 절대 못 잡았을 배포 환경 특유의 문제 3개**를 실제로 만나고 고침 — 이번 세션에서 "코드 리뷰"와 "실제로 돌려보기"가 얼마나 다른지 다시 확인된 사례:

1. **Cloudflare Pages 바인딩/환경변수는 저장만으로는 안 됨, 새 배포가 있어야 반영됨**: 대시보드에서 D1 바인딩을 저장한 직후 `/api/credits/state`를 호출하면 `error code: 1101`(Worker가 예외로 죽음, `env.DB`가 `undefined`)이 남 — 이미 빌드된 배포는 그 시점의 바인딩 스냅샷을 그대로 씀. **빈 커밋(`git commit --allow-empty`)을 만들어 push해서 재배포를 강제로 트리거**해 해결. 코드 변경 없이 배포만 새로 트리거하는 패턴으로, 앞으로 바인딩/환경변수를 바꿀 때마다 필요함.
2. **Paddle 상품의 `tax_category: 'digital-goods'`가 이 계정에서 기본 승인 상태가 아님**: `scripts/setup-paddle.mjs` 최초 실행이 `product_tax_category_not_approved` 400 에러로 실패. `'standard'`로 바꾸니 바로 통과 — 계정마다 사전 승인된 세금 카테고리가 다를 수 있다는 걸 실제로 겪고서야 알게 됨 (문서에는 사전승인 여부 목록이 명시돼 있지 않았음).
3. **Paddle 계정에 "Default payment link"가 설정돼 있지 않으면 오버레이 체크아웃 자체가 안 뜸**: `Checkout.open()`을 호출하면 겉보기엔 정상 로드되는 듯하다가 `transaction_default_checkout_url_not_set` 400으로 "Something went wrong" 화면이 뜸. 이건 **Paddle API로 설정할 방법이 없는 대시보드 전용 설정**(Paddle > Checkout > Checkout settings > Default payment link, Sandbox/Live 별도) — 사용자에게 정확한 메뉴 경로를 안내해서 처리함.

**실제 결제 테스트(Playwright, 헤드리스 브라우저로 진짜 Paddle 샌드박스 체크아웃을 끝까지 진행)**: `https://getohaeng.com/credits` 접속 → 1크레딧($0.99) 팩 클릭 → 실제 Paddle 오버레이(`sandbox-buy.paddle.com`, 카드 입력 필드는 Paddle 자체 프레임(`paddle_frame`)에 있고 Stripe iframe들은 Google Pay 전용이라 카드 직접입력엔 관여 안 함 — `#cardNumber`/`#cardHolder`/`#expiry`/`#cvv`) → 샌드박스 테스트카드 `4242 4242 4242 4242` 입력 후 결제 → Paddle이 자체 성공 화면 표시. 검증은 UI만 믿지 않고 3중으로 확인: ① 클라이언트 `checkout.completed` 이벤트가 실제로 발화해서 페이지가 "You're all set — Credits: 2"로 전환됨 ② `/credits` 표시 잔액이 1(웰컴)→2(구매 후)로 실제로 올라감 ③ **`wrangler d1 execute ohaeng-credits --remote`로 진짜 프로덕션 D1을 직접 조회**해서 `credits` 테이블 balance=2, `processed_transactions` 테이블에 진짜 Paddle 트랜잭션ID(`txn_01m0yzk9sfjjt0mwjnxrh2k6sn`, 내가 지어낸 값이 아니라 Paddle이 실제로 발급한 값)가 credits_added=1로 기록된 것까지 확인. 콘솔 에러 0건. 이전 세션에서 이미 검증했던 "가짜 웹훅 페이로드 curl 테스트"(서명 검증/중복지급 방지)와 "첫 방문 웰컴 크레딧" 항목도 이번엔 로컬 mock이 아니라 **실제 배포된 프로덕션 서버 + 실제 원격 D1**을 대상으로 재확인함.

## 13-2. saju/compatibility/romance도 크레딧 시스템으로 전환 — `PremiumContext` 실사용 종료

idolMatch/dramaMatch에서 검증된 크레딧 시스템(13, 13-1)을 나머지 3개 상품에도 확장 적용 — 이제 앱의 유료 콘텐츠 5개 전부(saju/compatibility/idolMatch/dramaMatch/romance)가 하나의 크레딧 잔액을 공유함. 상품 전체를 통째로 여닫던 `PremiumContext`/`utils/premiumUnlock.js` 방식은 이 3개에서도 더 이상 안 쓰임 — 사용자 요청대로 파일은 안 지우고 호출부만 바꿈.

- **itemKey 설계 — "사람(생년월일) 조합" 단위로 잠금**: `src/utils/credits.js`에 `birthKey(birth)`(내부 헬퍼: `{y}-{m}-{d}-{h}-{cal}`, `timeKnown`이 false면 시간 부분 생략 — `name`/`gender`는 리딩 내용에 영향 없어서 키에서 제외) 추가하고, 이를 조합한 3개 신규 export: `sajuItemKey(birth)` = `saju:{birthKey}`, `compatItemKey(myBirth, otherBirth)` = `compat:{myKey}:{otherKey}`(관계 라벨/이름은 키에 안 넣음 — 콘텐츠가 오행 관계로만 갈리므로), `romanceItemKey(situation, myBirth, otherBirth)` = `romance:{situation}:{myKey}:{otherKey}`(situation은 콘텐츠 뱅크 자체가 다른 축이라 키에 포함). 서버 쪽 `functions/api/credits/consume.js`의 `ITEM_KEY_RE`도 `idol`/`actor` 두 프리픽스만 허용하던 걸 `saju`/`compat`/`romance` 3개 추가 + 콜론 구분자 허용하도록 정규식 확장.
- **적용 범위 — 상품당 크레딧 1개가 그 사람(들)의 전체 리딩을 한 번에 잠금 해제**: `Saju.jsx`는 도메인 챕터 4개+십성챕터+extraSections+대운/인생그래프까지 총 5곳의 잠금을 전부 동일한 `sajuKey`(생년월일 하나당 하나)로 통일 — 하나 풀면 그 생년월일의 모든 섹션이 한 번에 풀림(섹션별 개별 과금 아님). `Compatibility.jsx`/`Romance.jsx`는 원래도 단일 `InsightSection` 호출이라 `product="compatibility"`/`"romance"` 자리를 각각 `unlockKey={compatItemKey(...)}`/`unlockKey={romanceItemKey(...)}`로 교체하는 것만으로 끝남. `InsightSection.jsx`는 이미 `unlockKey` 유무로 `CreditLock`/`PremiumLock`을 분기하는 구조였어서(3-13 이전 작업) 컴포넌트 자체는 무수정.
- **Guide.jsx 프리미엄 테스트 패널 정리**: `PRODUCTS.map()`으로 saju/compatibility/romance 3개를 개별 토글하던 UI를 통째로 제거하고(이 3개가 `PRODUCTS`의 전부였어서, 걸러내면 패널이 통째로 비어버림), 크레딧 시스템으로 옮겨갔다는 안내 문구 + 로컬 개발 시 `PREVIEW_MODE_UNLOCK_ALL`(전체 우회 스위치) 안내로 대체. `usePremium`/`PRODUCTS` import 제거.
- **`matchCommon.creditLock.note` 문구를 상품 불문 범용으로 수정**: "Unlock this member's full reading"(아이돌 전용 문구)이던 걸 "Unlock this full reading"으로 바꿔서 saju/compatibility/romance에서도 어색하지 않게 함(en/ko).
- **검증**: 실제 프로덕션에 영향 안 주려고 이번엔 로컬 `wrangler pages dev` + 로컬 D1(임시 `wrangler.toml`, 테스트 끝나고 삭제)에 테스트용 anonId를 10크레딧으로 시드해서 진행. Playwright로 (1) `/saju` 생년월일 A에서 크레딧 소비 → 19개 섹션(도메인 12+십성 3+extra 4) 전부 한 번에 잠금 해제 확인 → 생년월일 B로 바꾸면 19개 전부 다시 잠김 확인 → A로 복귀하면 잠금 해제 유지 + 서버 잔액 불변(재차감 없음) 확인 (2) `/compatibility` (나,X) 조합 해제 후 (나,Z)로 바꾸면 재잠김, (나,X)로 복귀하면 재차감 없이 유지 확인 (3) `/romance` situation=reunion에서 (나,X) 해제 후, **같은 생년월일 조합이라도 situation=crush로 바꾸면 재잠김**(콘텐츠 뱅크가 달라서 의도된 동작) 확인, reunion으로 복귀 시 재차감 없음 확인 (4) 크레딧 잔액이 saju/compatibility/romance/idolMatch/dramaMatch 전부 동일한 `anon_id` 기준 D1 `credits` 테이블 한 행을 공유하는 구조라 풀 공유가 아키텍처상 보장됨을 확인. en/ko(크레딧 잠금 안내 문구·버튼 텍스트 포함) 렌더링 확인, 콘솔 에러 0건. 테스트 후 임시 `wrangler.toml`/`.wrangler` 로컬 상태 삭제, git에 아무것도 안 남음.
