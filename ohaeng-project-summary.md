...
## 13. Paddle 크레딧 결제 시스템 (아이돌매치/드라마매치 멤버별 언락) — 코드 완료, 계정/대시보드 설정 대기 중
...
- **범위를 의도적으로 좁힘**: saju/compatibility/romance 3개는 기존 `PremiumContext`(상품 전체를 한 번에 영구 언락) 그대로 두고, **idolMatch/dramaMatch 2개만 크레딧 소비형으로 전환** — 크레딧 1개 = 특정 멤버/배우 한 명(예: `idol:bts-rm`, `actor:song-joongki`)의 전체 리딩 영구 언락. `utils/premiumUnlock.js`의 `PRODUCTS`에서 `idolMatch`/`dramaMatch` 제거함(Guide.jsx 재잠금 테스트 패널도 자동으로 3개만 보임).
